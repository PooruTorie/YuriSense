...
export default class SensorUpdater {
    public static async update(sensor: Sensor) {
        ...
        esp.on("state", state => RealtimeRouter.events.emit("send", "update_state", {state}));
        esp.on("progress", (current, total) => RealtimeRouter.events.emit("send", "update_progress", {current, total}));
