...
export default class LoginRetryQuery extends QueryCollection {
   ...
   async getRetryDataByIP(ip: string): Promise<{
      ...
      const [rows, fields] = await this.connection.query<RowDataPacket[]>(
         'SELECT * FROM LoginRetries WHERE ip = :ip',
         { ip },
      )
      if (rows.length === 0) {
         return null
   ...
   async resetRetryCount(ip: string): Promise<boolean> {
      return !(await this.connection.execute(
         'UPDATE LoginRetries SET retries=0, waitTimeMinutes=0 WHERE ip=:ip',
         {
            ip: ip,
         },
      ))
   ...
   async initWaitTime(ip: string) {
      return !(await this.connection.execute(
         'UPDATE LoginRetries SET waitTimeMinutes=1 WHERE ip=:ip',
         {
            ip: ip,
         },
      ))
