...
export default class SensorSettings extends Component {
    ...
    componentDidMount() {
        ...
        this.eventSource.addEventListener("update_state",
            e => {
                const data = JSON.parse(e.data);
                this.setState({updateLog: [...this.state.updateLog, <p>State: {data.state}</p>]});
            }
        );
        this.eventSource.addEventListener("update_progress",
            e => {
                const data = JSON.parse(e.data);
                this.setState({
                    updateProgress: data
    ...
    render() {
        return <Card>
            ...
            {this.state.update || !!this.state.updateError ? <Callout
                className="mt-4 mb-3"
                title={!this.state.updateError ? "Update Log" : "Error: " + this.state.updateError}
                color={!this.state.updateError ? "teal" : "red"}
            >
                <ProgressBar percentageValue={this.state.updateProgress.current * 100 / this.state.updateProgress.total}
                             color="teal" className="mt-3"/>
            ...
            <Text>
                ...
                {this.props.sensor.version !== this.state.newestVersion ?
                    <>
                        ...
                        &nbsp;&nbsp;
                        <Button variant="secondary" loading={this.state.update} icon={ArrowCircleUpIcon}
                                onClick={() => {
            ...
            <TextInput value={this.state.speed}
            ...
            <Button className="mt-2" disabled={!!this.state.nameError} loading={this.state.loading}
                    onClick={this.update.bind(this)}>Apply</Button>
        </Card>
    }
